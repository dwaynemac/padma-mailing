class Mailchimp::SubscriptionsController < Mailchimp::PetalController
  ...
  def new
    ...
    register_on_intercom('entered-new-mailchimp-subscription')
  end
  ...
  def create
    ...
    if created_id
      register_on_intercom('created-mailchimp-subscription')
  ...
  def destroy
    ...
    if @petal_subscription
      ...
      register_on_intercom('canceled-mailchimp-subscription')
    end
  ...
  def get_next_invoice_date
    response = Typhoeus.get("https://padma-accounts.herokuapp.com/v0/invoices?token=#{PadmaAccount.api_key}&account_id=#{current_user.current_account.name}&search[covers_until_gt]=#{Time.zone.today}")
    if response.success?
